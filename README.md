...
> land. v0.2 adds the contracts the v0.1 review asked for: global claim identities and
...
| `recalled` | journal / fact ids | **Model derivation** — every Muninn memory that was in the model's context when this entry was produced. An entry that restates a recalled fact is an *echo*, not corroboration ([self-reinforcement](#self-reinforcement)). |
| `used` | journal / fact ids | Subset of `recalled` that the outcome entry names as having mattered; the only input to `use_count`. |
...
- **Run tests with `pnpm test --run`, never watch mode.** id: f-testing-mbp-0007 · valid_from: 2026-08-22 · source: user · evidence: j-20260822-mbp-k7f3-0012 · cue: CI hangs on vitest
- **Integration tests need `DATABASE_URL` pointing at the docker compose db.** id: f-testing-ops1-0003 · valid_from: 2026-07-30 · source: tool · evidence: j-20260730-mbp-c2d8-0002, j-20260802-ops1-e4a0-0001

## Superseded

- ~~Tests run with `pnpm test`.~~ id: f-testing-mbp-0005 · valid_from: 2026-08-01 · valid_to: 2026-08-22 · superseded_by: f-testing-mbp-0007 · reason: user correction — watch mode hangs CI
```

A fact line is one bullet: bolded claim, then a ` · `-separated flat trailer. Facts have
stable ids (`f-<topic>-<host>-<seq>`, allocated by the dreaming host, never reused — so two
hosts' dreams can never mint the same id and a [merge](#merging-dreams) never has to
renumber). Supersession moves the
line to `## Superseded` with `valid_to`, `superseded_by` and `reason`; nothing is deleted by
a dream.

**Supersession reaches the journal.** A superseded fact's evidence is still in the
immutable journal, and a search over the journal would happily return it — or a later dream
re-promote it. So the dream also appends to `supersessions.md`, one line per invalidated
claim:

```markdown
- j-20260819-mbp-a91c-0007 · valid_to: 2026-08-22 · by: j-20260822-mbp-k7f3-0012 · fact: f-testing-mbp-0005
```
...
`memory_search` and the gather phase — is **active-only**: it drops journal chunks and facts
...
**Identities.** An entry id is `j-<yyyymmdd>-<host>-<sess>-<seq>`: the local date, the
host's short name (registered in `store.md`, unique per store), a 4-character hash of the
pi session id, and a per-session sequence. Two hosts, or two pi sessions on one host, can
therefore never allocate the same id — uniqueness needs no coordination beyond the host
registry. Ids are stable across sync and are what topic facts, `supersessions.md` and
`used:` fields cite. Timestamps are local with offset and are for humans; ordering is by id
within a session and by timestamp across sessions.

**Appends.** Journal files are per day *and per host*, so sync never has to merge two hosts'
writes to one file. Within a host, several pi sessions append to the same daily file; each
append takes an advisory lock (`journal/.lock`, lockfile with pid and timestamp, stale after
30 s), writes the whole entry with one `write`, `fsync`s, and releases. The id is allocated
from the session's own counter, so the lock protects file integrity only, never the id. A
...
2. **Concurrent capture is allowed.** Sessions keep appending to `journal/` in the working
   tree and committing to `main`. Those entries are simply *not yet dreamed*; the next dream
   picks them up. The worktree never sees them and never needs to.
3. **Remember is rebase-then-swap.** `main` may have advanced past `input_head` by journal
   commits (and by nothing else — capture touches only `journal/`; a dream touches only the
   derived paths). Remember rebases the dream branch onto `main`, which is conflict-free by
   construction for that case, and then updates `main` with a compare-and-swap
   (`git update-ref refs/heads/main <new> <expected-old>`). If the swap fails — another
   dream was remembered meanwhile — the rebase is retried once against the new head; a
   genuine conflict in a derived file (two dreams rewrote the same topic) is not left to
   `git merge`: it goes through the [merge dream](#merging-dreams), which is the
   consolidate phase with two candidates instead of one.
4. **A dream never rewrites `main`.** Fast-forward or CAS only; `forget` is a revert commit.
...
- Two hosts dreaming from *different* journals produce two rewrites of the same topic file.
  ...
  had.
- A fact that cites `j-…-ops1-…` on a host that has no `ops-1` journal is unsourced, and
  lint would rightly reject it.
...
remember it. Remember uses the same [rebase-then-swap](#the-journaldream-transaction) as
...
1. **Structural 3-way merge, per fact.** Topic files are lists of id'd bullets, so the
   ...
   Ids cannot collide because they carry the host.
...
registry (`host: mbp · added: 2026-08-22 · platform: darwin`). Every file format above —
...
- Crash recovery: a capture append is lock + single `write` + `fsync` (a truncated tail is
  ...
  collected by the next dream after the lock goes stale; remember is a single atomic ref
  update; sync is ordinary git and resumable by re-running. No step has a partial state that
...
| Ids that need no coordination | `host + session + seq` (journal) and `topic + host + seq` (facts) are unique by construction; a per-host counter or a daily sequence is not, and a UUID is unreadable in a diff. |
...
files with locked appends and coordination-free ids, capture-target scope, journal commits,
...
journal→dream transaction with rebase-then-swap remember; fact-level 3-way merge and the
