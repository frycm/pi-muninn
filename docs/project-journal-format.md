...
- `outcome`: bounded account of work, generated by the configured memory model and labelled
...
Phase 8 memories retain schema 1. Their `body` starts with `Memory format: issue-solution:v1`
or `Memory format: outcome:v1`, followed by readable sections and an evidence-ID list. Tags
include the phase and `memory:issue-solution:v1` or `memory:outcome:v1`. Issue sections cover
symptom, cause, solution, failed attempts, verification and applicability. Unknowns remain
explicit. These are body conventions, not new top-level fields; old clients and old
unstructured records remain compatible. Checkpoints use the same body conventions for
intermediate chunks. Settlement alone does not establish `status: completed`.
