...
| --- | --- | --- |
| Synced shard/directory link → scan or append → external file | `src/store/tracked-files.ts` rejects unsafe Git modes before checkout and revalidates after rebase. `src/journal/files.ts` checks directories and opens regular shards without following links; initialization uses the same directory guard. | `test/unit/sync.test.ts` checks clean/diverged histories, fetch-only runs and disabled symlink checkout. `test/unit/journal-files.test.ts` preserves outside files, rejects dangling links, directory links and FIFOs, and verifies store aliases, ordinary append and interrupted-tail recovery. |
