...
export const USAGE = [
	...
	"",
	"  /muninn                          status: scopes, journal, index, sync",
...
async function search(args: string, runtime: CommandRuntime): Promise<CommandOutput> {
	const { values, rest } = parseFlags(args, { valued: ["limit"] });
	if (rest === "") return { level: "warning", text: "muninn: /muninn search [--limit n] <query>" };
	...
	const hits = runtime.indexes()?.search({ query: rest, limit }) ?? [];

	if (hits.length === 0) {
		return {
			level: "info",
			text: `muninn: nothing matches "${rest}" — try different words`,
		};
	}
	...
	const lines = hits.map((hit) => `  ${renderHitLine(hit)}`);
	const header = `${hits.length} journal ${hits.length === 1 ? "record" : "records"} for "${rest}":`;
