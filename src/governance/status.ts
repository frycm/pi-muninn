...
export interface CryptographicStatus {
	...
	key_events: { total: number; states: Record<VerificationState, number> };
	warnings: string[];
}
...
export function cryptographicStatus(options: {
	...
	const keyStates = counts();
	for (const event of options.manifest.key_events) keyStates[projection.keyEvent(event)]++;
	...
	return {
		...
		key_events: { total: options.manifest.key_events.length, states: keyStates },
...
export function renderCryptographicStatus(status: CryptographicStatus): string[] {
	...
	return [
		...
		`key events: ${nonzero(status.key_events.states)}`,
